...
async function main() {
  ...
  try {
    ...
    // Pastikan risk.entry valid
    let entryData = risk.entry;

    // Jika risk.entry undefined atau tidak punya .price, buat fallback
    if (!entryData || typeof entryData !== 'object' || !entryData.price) {
    ...
    const state = analyzeState(
      Number(last.close),  // price
      entryData,           // entry (object dengan .price)
      atr14                // atr
    );
    ...
    console.log("Confidence  :", execution.confidence != null ? execution.confidence + "%" : "-");
    ...
    console.log("Stop Loss     :", execution.stopLoss ?? "-");
    console.log("Target        :", execution.target ?? "-");
    console.log("Risk Reward   :", execution.riskReward > 0 ? execution.riskReward : "-");
